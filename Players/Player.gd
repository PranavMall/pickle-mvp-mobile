...
# Note: Main.gd now creates players dynamically, this script is for standalone use
...
func _ready() -> void:
	...
	if not main or not ball:
		push_warning("Player cannot find Main or Ball - may be managed by Main.gd")
		return
...
func create_visuals() -> void:
	...
	var player_color = Color(0.2, 0.4, 0.8)
	...
	if not paddle:
		...
		paddle_img.fill(Color(0.5, 0.3, 0.1))
...
func _physics_process(delta: float) -> void:
	...
	# Only update if ball reference is valid
...
func update_movement(delta: float) -> void:
	...
	if main.game_state.has("kitchen_mastery") and main.game_state.kitchen_mastery:
		speed *= 1.3

	# Get ball court position
	if not ball or not ball.has_method("screen_to_court"):
		return
	...
	if ball_on_our_side and ball.in_flight:
		var time_to_reach = max(0, (ball.position.y - position.y) / max(abs(ball.velocity.y), 1))
	...
	clamp_to_court_bounds()

func clamp_to_court_bounds() -> void:
	var court = main.get_node_or_null("Court")
	if not court or not court.has_method("screen_to_court"):
		return

	var court_pos = court.screen_to_court(position)
...
func should_cover_ball(predicted_x: float) -> bool:
	...
	var in_my_zone = (court_side == "right" and predicted_x > viewport_center) or \
					 (court_side == "left" and predicted_x <= viewport_center)
	return in_my_zone

func calculate_intercept_position(predicted_x: float, ball_y: float) -> Vector2:
	var intercept_x = predicted_x
	var intercept_y = ball_y + 30
...
func update_kitchen_status(delta: float) -> void:
	...
	if court and court.has_method("screen_to_court"):
		court_pos = court.screen_to_court(position)
...
func _on_body_entered(body: Node2D) -> void:
	if body == ball and can_hit:
		pass  # Swipe system handles hitting

func enter_kitchen() -> void:
	if not main:
		return

	# Check if kitchen state exists and is available
	var kitchen_available = false
	if main.game_state.has("kitchen_state"):
		kitchen_available = main.game_state.kitchen_state == main.KitchenState.AVAILABLE
