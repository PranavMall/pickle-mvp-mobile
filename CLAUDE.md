...
**Known bugs from Day 5:**
- Pressure does not reset to zero after mastery mode activation
- Duplicate percentage labels appear on the mastery button
...
1. **Fix Day 5 bugs** — Pressure reset + duplicate mastery labels
2. **GUT testing framework** — Install GUT 9.x, write requirement tests
3. **Paddle system** — Visual paddles on all 4 characters with swing animations and hit detection
4. **Sound design** — 5+ hit sound variations with power scaling, ambient court audio, crowd reactions, UI sounds
5. **AI enhancement** — Smarter kitchen strategy, difficulty scaling, better shot selection
6. **Visual polish** — Ghibli-inspired art via canvas_item shaders, cherry blossom particles, warm palette
7. **Tutorial** — Step-by-step onboarding teaching serve, rally, kitchen, and mastery
8. **Menu system** — Main menu, settings, game flow (menu → game → results → menu)
9. **Performance optimization** — Object pooling, draw call reduction, <150MB RAM, <10% battery/hour
10. **Multiplayer foundation** — Turn-based state sync, peer-to-peer networking
11. **Store submission** — APK/iOS builds, store listings, final polish
...
This project uses **GUT 9.x** (Godot Unit Test) for automated testing. Testing is not optional.

### Setup
- GUT addon lives in `addons/gut/`
- Tests live in `res://test/` with subdirectories: `unit/`, `integration/`, `gameplay/`
- Config at `res://.gutconfig.json`

### Rules — Follow Every Time
1. **Write tests BEFORE implementation** for any new feature
2. After EVERY code change, run:
   ```
   godot --headless -s addons/gut/gut_cmdln.gd -gdir=res://test -ginclude_subdirs -gexit
   ```
3. **ALL tests must pass** before proceeding to the next task
4. If tests fail, **fix and re-run until green** — do not move on with failing tests
5. Never skip tests. Never mark tests as "will fix later." Never delete a failing test to make the suite pass.
6. The prototype (`prototype_ver_2_1.txt`) is the **source of truth** for all game rules
7. Do not modify existing passing tests without asking the developer first

### What Must Be Tested
- **Scoring**: score-on-serve, server rotation, side out, win conditions
- **Service**: diagonal validation, kitchen fault, out of bounds, wrong box
- **Double Bounce**: first bounce rule, second bounce rule, volley allowed after both
- **Kitchen**: all 6 state transitions, all 4 violation priority checks, momentum carry
- **Pressure & Mastery**: pressure values, mastery activation, reset, duration, benefits
- **Ball Physics**: gravity, bounce, friction, height bounds, shadow scaling, trail length
- **AI**: positioning, court coverage, reaction timing, kitchen entry decisions

### Verification Loop
```
implement → run tests → FAIL? → fix → run tests → PASS? → commit
                                  ↑                          |
                                  └──────────────────────────┘
```

**Claude Code must not consider any task "done" until all tests pass.**
